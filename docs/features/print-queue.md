...
This is the same filament mapping panel used when reprinting from Archives — including the **Mapping** button that reuses a slicer-saved AMS pick. See [Reuse the Slicer's Saved Mapping](archiving.md#reuse-the-slicers-saved-mapping) for the full explanation.
