...
- **A 3MF that wants supports can keep them.** Bambu's shipped process presets all have supports off, because that is a decision per print rather than per quality level. So when a 3MF's own settings switch supports on, that choice &mdash; along with its support and interface filament slots, and tree versus normal &mdash; can be carried onto the preset you picked, and a file exported with PVA in the interface slot still slices that way. Tick **Enable support** where it is marked *from file*, or use **Use the file's built-in settings** above the panel. The carry only ever switches supports **on**: a file that has them off leaves your preset's own support settings alone, so a preset that deliberately enables them is not overruled by a download that does not use them. Where a file records no deviations for you to tick at all &mdash; an OrcaSlicer export, for instance &mdash; its support configuration is carried without asking, as it always was. Either way an explicit choice in this panel wins, and the slice log names any setting that was carried.
...
They appear **in this panel, against the options they belong to**, each marked *from file* with a tick box for whether to use it.

**Nothing is ticked for you.** These ticks follow the **Use the file's built-in settings** checkbox above them: with it off, no setting comes out of the file unless you ask for it by name; with it on, the file drives the whole slice and every one of them shows ticked. A line at the top of the panel says how many settings the file changed and offers to take them in one click. Two groups it leaves for you to decide individually:
