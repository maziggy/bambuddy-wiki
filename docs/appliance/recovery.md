...
The appliance wipes its user data, forgets the first-boot marker, and reboots into a fresh setup wizard &mdash; exactly as a new appliance would. This is the path of last resort, and it works when nothing else does: when the network is misconfigured, when the password is lost, when the web interface is unreachable, when both ethernet and WiFi have failed.

| Erased | Kept |
|---|---|
| Bambuddy's database &mdash; printers, queue, history, users | The operating system and Bambuddy itself |
| All uploads &mdash; gcode, snapshots, timelapses | Partner branding |
